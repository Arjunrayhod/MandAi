...
export const RathoreClassicTemplate: React.FC<InvoiceTemplateProps> = ({ invoice, company }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start pb-2 border-b border-gray-300">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            {company.phoneExtra && (
              <p className="text-gray-800 text-[10.5px] font-medium">{company.phoneExtra}</p>
            )}
            {company.mandiLicenseNo && (
              <p className="text-gray-700 text-[9.5px]">मंडी अनुज्ञप्ति क्र.: {company.mandiLicenseNo}</p>
            )}
          </div>
        </div>

        {/* Right: Contact Details */}
        <div className="text-right text-[10px] text-gray-800 space-y-0.5">
          <p><span className="font-semibold text-black">Name :</span> {company.ownerName || 'Kundan Rathore'}</p>
          <p><span className="font-semibold text-black">Phone :</span> {company.phone || company.email}</p>
          <p><span className="font-semibold text-black">Email :</span> {company.email}</p>
          <p><span className="font-semibold text-black">Website :</span> {company.website || 'www.rathoretradingcompany.co.in'}</p>
        </div>
      </div>

      {/* 2. Sub-Header Bar (GSTIN | TAX INVOICE | ORIGINAL FOR RECIPIENT) */}
      <div className="grid grid-cols-3 items-center py-1.5 px-2 bg-gray-50 border-x border-b border-black font-semibold text-[11px]">
        <div>
          <span className="font-bold">GSTIN :</span> {company.gstin}
        </div>
        <div className="text-center text-[#0284c7] text-base font-extrabold tracking-wider">
          TAX INVOICE
        </div>
        <div className="text-right text-[10px] uppercase font-bold tracking-wide">
          ORIGINAL FOR RECIPIENT
        </div>
      </div>

      {/* 3. Customer & Invoice Info Table */}
      <div className="grid grid-cols-2 border-x border-b border-black text-[10.5px]">
        {/* Left Column: Customer Details */}
        <div className="border-r border-black">
          <div className="bg-[#f0f9ff] text-center font-bold py-1 border-b border-black text-[11px] text-gray-900">
            Customer Detail
          </div>
          <div className="p-2 space-y-1">
            <div className="flex">
              <span className="font-bold w-16 shrink-0">M/S</span>
            ...
            <div className="flex items-start">
              ...
              <span className="text-gray-800">{invoice.billingAddress || party.billingAddress}</span>
            </div>
            <div className="flex">
              <span className="font-semibold w-16 shrink-0">Phone</span>
              <span className="text-gray-800">{party.phone || '-'}</span>
            </div>
            <div className="flex">
              <span className="font-semibold w-16 shrink-0">GSTIN</span>
              <span className="font-bold text-gray-900">{party.gstin || '-'}</span>
            </div>
            <div className="flex">
              <span className="font-semibold w-16 shrink-0">PAN</span>
              <span className="text-gray-800">{party.pan || (party.gstin ? party.gstin.substring(2, 12) : '-')}</span>
            </div>
            <div className="flex">
              <span className="font-semibold w-16 shrink-0">Place of Supply</span>
        ...
        <div>
          <div className="p-2 space-y-2">
            <div className="flex justify-between items-center py-0.5 border-b border-gray-200">
              ...
              <span className="font-bold text-black text-sm">{invoice.invoiceNumber}</span>
            </div>
            <div className="flex justify-between items-center py-0.5 border-b border-gray-200">
            ...
            {invoice.vehicleNo && (
              <div className="flex justify-between items-center py-0.5 border-b border-gray-200">
                <span className="font-semibold text-gray-700">Vehicle / Gadi No.</span>
                <span className="font-medium">{invoice.vehicleNo}</span>
              </div>
            )}
            {invoice.biltyNo && (
              <div className="flex justify-between items-center py-0.5">
                <span className="font-semibold text-gray-700">Bilty / GR No.</span>
                <span className="font-medium">{invoice.biltyNo}</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-12 border-x border-b border-black text-[10px]">
        ...
        <div className="col-span-7 border-r border-black flex flex-col justify-between">
          ...
          <div className="border-b border-black">
            <div className="bg-[#f0f9ff] text-center font-bold py-0.5 border-b border-black text-[10.5px]">
              Total in words
            </div>
            <div className="p-1.5 font-bold text-gray-900 leading-snug">
              {totalInWordsText}
            </div>
          </div>
          ...
          <div className="border-b border-black">
            ...
            <div className="p-2 grid grid-cols-3 gap-2 items-center">
              <div className="col-span-2 space-y-0.5 text-[10px]">
                <p><span className="font-semibold">Name :</span> {company.bankDetails?.bankName || 'HDFC Bank'}</p>
                <p><span className="font-semibold">Branch :</span> {company.bankDetails?.branch || 'Vijay talkies compound neemuch'}</p>
                <p><span className="font-semibold">Acc. Name :</span> <span className="font-bold">{company.bankDetails?.accountName || company.name}</span></p>
                <p><span className="font-semibold">Acc. Number :</span> <span className="font-bold font-mono">{company.bankDetails?.accountNumber || '50200098211151'}</span></p>
                <p><span className="font-semibold">IFSC :</span> <span className="font-mono font-bold">{company.bankDetails?.ifsc || 'HDFC0000624'}</span></p>
                <p><span className="font-semibold">UPI ID :</span> <span className="text-blue-700 font-medium">{company.bankDetails?.upiId || '9340829951@hdfcbank'}</span></p>
              </div>
          ...
          {/* Terms & Conditions */}
          <div className="p-2 text-[9px] text-gray-700 space-y-0.5">
        ...
        <div className="col-span-5 flex flex-col justify-between">
          <div className="text-[10px]">
            ...
            <div className="flex justify-between py-1 px-2 border-b border-gray-300">
              <span className="text-gray-800 font-medium">Transport charges</span>
              <span className="font-semibold">{(invoice.transportCharges || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>
            <div className="flex justify-between py-1 px-2 border-b border-gray-300">
              <span className="text-gray-800 font-medium">{invoice.otherChargesLabel || 'All other charges (कट्ट)'}</span>
              <span className="font-semibold">{(invoice.otherCharges || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>
            <div className="flex justify-between py-1 px-2 border-b border-black bg-gray-50 font-bold">
            ...
            {!isInterState ? (
              <>
                <div className="flex justify-between py-1 px-2 border-b border-gray-200">
                  <span className="text-gray-800 font-medium">Add : CGST</span>
                  <span className="font-semibold">{invoice.totalCgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                </div>
                <div className="flex justify-between py-1 px-2 border-b border-gray-200">
                  <span className="text-gray-800 font-medium">Add : SGST</span>
                  <span className="font-semibold">{invoice.totalSgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                </div>
              </>
            ) : (
              <div className="flex justify-between py-1 px-2 border-b border-gray-200">
                <span className="text-gray-800 font-medium">Add : IGST</span>
                <span className="font-semibold">{invoice.totalIgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
              </div>
            )}

            <div className="flex justify-between py-1 px-2 border-b border-black font-bold">
              <span>Total Tax</span>
              <span>{invoice.totalTax.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>

            <div className="flex justify-between py-1.5 px-2 border-b-2 border-black bg-[#e0f2fe] text-black font-extrabold text-[12px]">
              <span>Total Amount After Tax</span>
              <span>{formatIndianCurrency(invoice.finalAmount)}</span>
            </div>
            <div className="text-right px-2 py-0.5 text-[9px] text-gray-600 italic">
              (E & O.E.)
            </div>
          </div>

          {/* Signatory Block */}
          <div className="border-t border-black text-center pt-1 pb-2 px-2">
            <p className="text-[8.5px] text-gray-700">Certified that the particulars given above are true and correct.</p>
            <p className="font-bold text-black text-[10.5px] mt-0.5">For {company.name}</p>
            <div className="h-10"></div>
            <p className="text-[9.5px] font-semibold border-t border-gray-400 inline-block px-4 pt-0.5 text-gray-800">
